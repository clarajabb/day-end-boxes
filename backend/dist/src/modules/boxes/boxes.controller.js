"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BoxesController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const boxes_service_1 = require("./boxes.service");
let BoxesController = class BoxesController {
    constructor(boxesService) {
        this.boxesService = boxesService;
    }
    async getNearbyBoxes(lat, lng, radius = 10) {
        const boxes = await this.boxesService.findNearbyBoxes(lat, lng, radius);
        return {
            success: true,
            data: boxes,
        };
    }
    async getBoxById(id) {
        const box = await this.boxesService.findBoxById(id);
        return {
            success: true,
            data: box,
        };
    }
};
exports.BoxesController = BoxesController;
__decorate([
    (0, common_1.Get)('nearby'),
    (0, swagger_1.ApiOperation)({ summary: 'Get nearby available boxes' }),
    __param(0, (0, common_1.Query)('lat')),
    __param(1, (0, common_1.Query)('lng')),
    __param(2, (0, common_1.Query)('radius')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, Number, Number]),
    __metadata("design:returntype", Promise)
], BoxesController.prototype, "getNearbyBoxes", null);
__decorate([
    (0, common_1.Get)(':id'),
    (0, swagger_1.ApiOperation)({ summary: 'Get box details' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], BoxesController.prototype, "getBoxById", null);
exports.BoxesController = BoxesController = __decorate([
    (0, swagger_1.ApiTags)('Boxes'),
    (0, common_1.Controller)('boxes'),
    __metadata("design:paramtypes", [boxes_service_1.BoxesService])
], BoxesController);
//# sourceMappingURL=boxes.controller.js.map